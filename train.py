...
def make_json(logits, qIds, dataloader):
    ...
    for i in range(logits.size(0)):
        ...
        result['max'] = logits[i].max().item()
        result['gt_pred'] = logits[i].argmax().item()
...
def train(model, train_loader, eval_loader,args,qid2type):
    ...
    for epoch in range(num_epochs):
        ...
        if run_eval:
            model.train(False)
            results = evaluate(epoch, model, eval_loader, qid2type)
...
def evaluate(epoch, model, dataloader, qid2type):
    score = 0
    upper_bound = 0
    score_yesno = 0
    score_number = 0
    ...
    for v, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        pre_max = pred.max(dim=1).values.tolist()
        gt_indices = a.argmax(dim=1)
        pred_right = pred[torch.arange(batch_size), gt_indices].tolist()
