...
class PlainCe(DebiasLossFn):
    def forward(self, hidden, logits, bias, labels, conf):

        # ce
        nll = F.log_softmax(logits, dim=-1)
        loss_ce = -nll * labels # * (1-conf)
        # conf
        prediction_ans_k, top_ans_ind = torch.topk(F.softmax(labels, dim=-1), k=1, dim=-1, sorted=False)
        loss_conf = torch.gather(F.softmax(logits, dim=-1), 1, top_ans_ind) * conf * 3.3
...
#         # ce
#         nll = F.log_softmax(logits, dim=-1)
#         #print(nll.size(), labels.size()) torch.Size([512, 3129]) torch.Size([512, 3129])
#         loss_ce = (-nll * labels).sum(dim=-1) # * (1-conf)
#         # conf
#         prediction_ans_k, top_ans_ind = torch.topk(F.softmax(labels, dim=-1), k=1, dim=-1, sorted=False)
#         loss_conf = torch.gather(F.softmax(logits, dim=-1), 1, top_ans_ind).sum(1) * conf * 10
#
#         loss = loss_ce + loss_conf
#
#         return loss.mean()
